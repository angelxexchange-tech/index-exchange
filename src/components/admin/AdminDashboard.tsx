...
export default function AdminDashboard({ adminUser, onLogout }: AdminDashboardProps) {
  ...
  const [balanceAsset, setBalanceAsset] = useState<"INR" | "USDT" | "TRX" | "BNB" | "USDT-BEP20">("INR");
    ...
    <div className="min-h-screen w-full bg-slate-950 text-slate-100 flex flex-col font-sans select-none overflow-x-hidden">
      ...
      <div className="flex-1 flex w-full relative">
        ...
        <main className="flex-1 p-4 space-y-2 w-full">
          ...
          {activeTab === "overview" && (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 flex flex-col justify-between shadow-md">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <div className="flex justify-between text-xs mb-1.5">
                        <span className="font-bold text-slate-200">TRX Balance</span>
                        <span className="font-mono text-rose-400">
                          {(stats?.systemBalances?.trx || 0).toLocaleString()} TRX
                        </span>
                      </div>
                      <div className="w-full h-2 bg-slate-950 rounded-full overflow-hidden">
                    ...
                    <div>
                      <div className="flex justify-between text-xs mb-1.5">
                        <span className="font-bold text-slate-200">BNB Balance</span>
                        <span className="font-mono text-amber-400">
                          {(stats?.systemBalances?.bnb || 0).toLocaleString()} BNB
                        </span>
                      </div>
                      <div className="w-full h-2 bg-slate-950 rounded-full overflow-hidden">
                        <div className="h-full bg-[#F5B301] rounded-full w-[15%]" />
                      </div>
                    </div>
                  </div>
                  ...
                  <button
                    type="button"
                    onClick={() => setActiveTab("wallets")}
                    className="w-full mt-6 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold transition-all cursor-pointer flex items-center justify-center space-x-1"
                  >
                    <span>View Wallet Breakdown</span>
              ...
              {(pendingDeposits.length > 0 || pendingWithdrawals.length > 0) && (
                <div className="bg-slate-900/90 border border-amber-500/40 rounded-2xl p-6 space-y-4 shadow-lg">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2.5">
                      <div className="w-3 h-3 rounded-full bg-amber-400 animate-ping" />
                      <h3 className="text-sm font-bold text-white">
                        Pending Approval Requests ({pendingDeposits.length + pendingWithdrawals.length})
                      </h3>
                    </div>
                    <span className="text-xs text-amber-400 font-medium">Requires Admin Sign-off</span>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[...pendingDeposits, ...pendingWithdrawals].slice(0, 4).map((t) => (
                      <div
                        key={t._id}
                      ...
                      >
                        <div className="space-y-1">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-xs font-bold text-white font-mono">{t.userId}</span>
                          </div>
            ...
            <div className="space-y-4 animate-in fade-in duration-300">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-400">Manage user accounts and wallet balances</p>
                </div>
              </div>
                  ...
                  <table className="w-full text-left text-xs text-slate-300">
                    <thead className="bg-slate-950 text-slate-400 uppercase font-bold border-b border-slate-800">
                      <tr>
                        ...
                        <th className="p-3.5">USDT Balance</th>
                        <th className="p-3.5">TRX / BNB</th>
                      ...
                      {filteredUsers.length === 0 ? (
                        ...
                        filteredUsers.map((u) => (
                          <tr key={u._id} className="hover:bg-slate-800/40 transition-colors">
                            <td className="p-3.5 font-bold font-mono text-[#31A9F6]">{u.userId}</td>
                            <td className="p-3.5 font-semibold text-white">{u.name}</td>
                            <td className="p-3.5 font-mono text-slate-400">{u.mobileNumber}</td>
                            <td className="p-3.5 font-bold text-emerald-400">
                              ₹{(u.wallet?.inrBalance || 0).toLocaleString()}
                            </td>
                            <td className="p-3.5 font-bold text-[#31A9F6]">
                              ${(u.wallet?.usdtBalance || 0).toLocaleString()} USDT
                            </td>
                            <td className="p-3.5 text-slate-300 font-mono">
                              {u.wallet?.trxBalance || 0} TRX | {u.wallet?.bnbBalance || 0} BNB
                            </td>
                            ...
                            <td className="p-3.5 text-right">
                              <button
                                ...
                                className="px-3 py-1 bg-[#31A9F6]/10 hover:bg-[#31A9F6]/20 border border-[#31A9F6]/40 text-[#31A9F6] font-semibold rounded-lg transition-all cursor-pointer"
                              >
                                Adjust Balance
                              </button>
                            </td>
          ...
          {/* TRANSACTIONS / DEPOSITS / WITHDRAWALS TAB */}
          {(activeTab === "transactions" || activeTab === "deposits" || activeTab === "withdrawals") && (
            <div className="space-y-4 animate-in fade-in duration-300">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h2 className="text-base sm:text-lg font-bold text-white capitalize flex items-center space-x-2">
                    <History className="w-5 h-5 text-[#31A9F6]" />
                    <span>
                      {activeTab === "deposits"
                        ? "Deposit Requests"
                        : activeTab === "withdrawals"
                  ...
                  <p className="text-xs text-slate-400">
                    Real-time transaction log & approval management
                  </p>
                </div>
                  ...
                  <div className="flex items-center space-x-1 bg-slate-900 border border-slate-800 p-1 rounded-xl">
                    {(["all", "pending", "completed", "rejected"] as const).map((st) => (
                      <button
                        key={st}
                        type="button"
                        onClick={() => setTxnFilter(st)}
                        className={`px-3 py-1 rounded-lg text-xs font-semibold capitalize transition-all cursor-pointer ${
                          txnFilter === st
                            ? "bg-[#31A9F6] text-white"
                  ...
                  <table className="w-full text-left text-xs text-slate-300">
                    <thead className="bg-slate-950 text-slate-400 uppercase font-bold border-b border-slate-800">
                      <tr>
                        ...
                        <th className="p-3.5">Address / Details</th>
                      ...
                      {filteredTransactions.length === 0 ? (
                        ...
                        filteredTransactions.map((t) => (
                          <tr key={t._id} className="hover:bg-slate-800/40 transition-colors">
                            <td className="p-3.5 font-bold font-mono text-slate-200">{t.referenceId}</td>
                            <td className="p-3.5 font-bold font-mono text-[#31A9F6]">{t.userId}</td>
                            <td className="p-3.5">
                              <span
                                className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase ${
                                  t.type === "deposit"
                            ...
                            <td className="p-3.5 font-extrabold text-white">
                              {t.amount} <span className="text-[#31A9F6]">{t.asset}</span>
                            </td>
                            <td className="p-3.5">
                              <span
                            ...
                            <td className="p-3.5 font-mono text-slate-500 max-w-[150px] truncate">
                              {t.address || "N/A"}
                            </td>
                            <td className="p-3.5 text-slate-500">
                              ...
                              {t.status === "pending" ? (
                                <div className="flex items-center justify-end space-x-1.5">
                                  <button
                                    ...
                                    className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-md text-[11px] font-bold transition-all cursor-pointer"
                                  >
                                    Approve
                                  </button>
                                  <button
                                    ...
                                    className="px-2.5 py-1 bg-rose-600 hover:bg-rose-500 text-white rounded-md text-[11px] font-bold transition-all cursor-pointer"
                                  >
                                    Reject
                                  </button>
                                </div>
          ...
          {activeTab === "wallets" && (
            <div className="space-y-4 animate-in fade-in duration-300">
              <div>
                <h2 className="text-base sm:text-lg font-bold text-white flex items-center space-x-2">
                  <Wallet className="w-5 h-5 text-[#F5B301]" />
                  <span>System Asset Liquidity Overview</span>
                </h2>
                <p className="text-xs text-slate-400">Total balances pooled across user accounts</p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
                  <div className="text-xs text-slate-400 font-semibold mb-1">INR Pool</div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
                  <div className="text-xs text-slate-400 font-semibold mb-1">USDT Pool</div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
                  <div className="text-xs text-slate-400 font-semibold mb-1">TRX Pool</div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
                  <div className="text-xs text-slate-400 font-semibold mb-1">BNB Pool</div>
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <form onSubmit={handleSaveRates} className="space-y-6 max-w-4xl">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  {/* TRX */}
                  <div className="bg-slate-900/90 border border-slate-800/90 rounded-2xl p-5 space-y-3 shadow-md hover:border-rose-500/40 transition-all">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 rounded-xl bg-rose-500/10 border border-rose-500/30 flex items-center justify-center font-bold text-rose-400">
                          TRX
                        </div>
                        <div>
                          <div className="text-sm font-bold text-white">TRX Rate</div>
                          <div className="text-[11px] text-slate-400">TRON Network Token</div>
                        </div>
                      </div>
                  ...
                  {/* BNB */}
                  <div className="bg-slate-900/90 border border-slate-800/90 rounded-2xl p-5 space-y-3 shadow-md hover:border-purple-500/40 transition-all">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <div className="text-sm font-bold text-white">BNB Rate</div>
                          <div className="text-[11px] text-slate-400">Binance Coin</div>
                        </div>
                      </div>
