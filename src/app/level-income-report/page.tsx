...
import { ArrowLeft, Search, X, Filter, Loader2, Award } from "lucide-react";
...
export default function LevelIncomeReportPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/user/income-reports?userId=${encodeURIComponent(userId)}&type=level`)
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
          setIncomeLogs(data.logs || []);
          setLevelIncomeTotal(data.wallet?.levelIncome || 0);
        }
      })
      .catch((err) => console.error("Error fetching level income:", err))
  ...
  const filteredLogs = incomeLogs.filter(
    (log) =>
      ...
      log.referenceTxId.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    <div className="relative flex flex-col w-full h-full min-h-screen bg-[#F0F2F5] overflow-x-hidden font-sans pb-12 select-none">
      <main className="flex-1 px-4 pt-4 pb-8 max-w-[430px] mx-auto w-full flex flex-col">
        ...
        {/* Total Summary Card */}
        <div className="w-full bg-gradient-to-r from-[#1C82D9] to-[#38B6FF] rounded-2xl p-4 text-white mb-5 shadow-sm flex items-center justify-between">
          <div>
        ...
        <div className="flex items-center space-x-2 mb-4">
          <div className="relative flex-1 bg-white rounded-full border border-slate-200/90 shadow-[0_2px_8px_rgba(0,0,0,0.04)] px-4 py-2.5 flex items-center">
            ...
            <input
              ...
              placeholder="Search by User ID or Ref ID"
        ...
        {loading ? (
              ...
              >
                <div className="flex flex-col">
                  <div className="flex items-center space-x-2">
                    <span className="px-2 py-0.5 bg-[#1C82D9]/10 text-[#1C82D9] text-[11px] font-bold rounded-md">
                      Level {log.level}
                    </span>
          ...
          <div className="flex-1 flex flex-col items-center justify-center pt-8 pb-16">
            ...
            <p className="text-[#A0A8B6] font-extrabold text-[15px] tracking-tight mt-6 text-center">
              No level income records available
            </p>
          </div>
        )}
      </main>
