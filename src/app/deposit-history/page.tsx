...
export default function DepositHistoryPage() {
  ...
  const [transactions, setTransactions] = useState<DepositTxn[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!isAuthenticated || !userId) return;

    fetch(`/api/user/transactions?userId=${encodeURIComponent(userId)}&type=deposit`)
      .then((res) => res.json())
      .then((data) => {
      ...
      .catch((err) => console.error("Error fetching deposit history:", err))
      ...
      <main className="flex-1 px-4 pt-4 pb-8 max-w-[430px] mx-auto w-full flex flex-col">
        ...
        <div className="relative w-full bg-white rounded-full border border-slate-200/90 shadow-[0_2px_8px_rgba(0,0,0,0.04)] px-4 py-2.5 flex items-center mb-6">
          ...
          <input
            ...
            placeholder="Search Ref ID, Asset..."
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-16 text-slate-400 space-y-2">
            ...
            <span className="text-xs">Loading deposit history...</span>
          </div>
