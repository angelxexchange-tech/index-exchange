...
import { ArrowLeft, Search, X, Loader2, DollarSign } from "lucide-react";
...
export default function TotalIncomeReportPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/user/income-reports?userId=${encodeURIComponent(userId)}&type=all`)
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
          setIncomeLogs(data.logs || []);
          setTotalIncome(data.wallet?.totalIncome || 0);
        }
      })
      .catch((err) => console.error("Error fetching total income:", err))
  ...
  return (
    <div className="relative flex flex-col w-full h-full min-h-screen bg-[#F0F2F5] overflow-x-hidden font-sans pb-12 select-none">
      <main className="flex-1 px-4 pt-4 pb-8 max-w-[430px] mx-auto w-full flex flex-col">
        ...
        {/* Total Summary Card */}
        <div className="w-full bg-gradient-to-r from-[#F5B301] to-[#F7C844] rounded-2xl p-4 text-slate-900 mb-5 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-slate-800 text-xs font-semibold uppercase tracking-wider block">
              Grand Total Income Earned
            </span>
            <span className="text-2xl font-extrabold mt-0.5 block text-slate-950">
        ...
        {loading ? (
              ...
              >
                    ...
                    <span
                      className={`px-2 py-0.5 text-[11px] font-bold rounded-md ${
                        log.type === "ltd"
                          ...
                          : "bg-[#1C82D9]/10 text-[#1C82D9]"
                      }`}
                    >
                      {log.type === "ltd" ? "LTD Income" : `Level ${log.level}`}
                    </span>
                ...
                <div className="flex flex-col items-end">
                  ...
                  <span className="text-slate-400 text-[10.5px]">
                    On ₹{log.transactionAmount} ({log.asset})
                  </span>
                </div>
              </div>
            ))}
          ...
          <div className="flex-1 flex flex-col items-center justify-center pt-8 pb-16">
            ...
            <p className="text-[#A0A8B6] font-extrabold text-[15px] tracking-tight mt-6 text-center">
              No total income records available
            </p>
          </div>
        )}
      </main>
